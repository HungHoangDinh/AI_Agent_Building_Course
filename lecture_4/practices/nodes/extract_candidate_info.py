...
class CandidateInfo(BaseModel):
    ...
    years_experience: Optional[int] = Field(default=None, description="Số năm kinh nghiệm")
    ...
    seniority: Optional[str] = Field(default=None, description="Bậc kinh nghiệm: Junior / Mid / Senior / Lead")
    main_skills: Optional[list[str]] = Field(default=None, description="Kỹ năng chính")
...
def extract_candidate_info_node(state: dict) -> dict:
    ...
    try:
        ...
        # TODO: Gọi LLM (OpenAI) với EXTRACT_CANDIDATE_INFO_PROMPT, parse JSON -> CandidateInfo; return {"candidate_info": info.model_dump()}
        # client = OpenAI(); response = client.chat.completions.create(...); parse response -> CandidateInfo
